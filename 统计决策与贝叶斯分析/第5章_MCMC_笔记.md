...
> 在 [[第11周星期五-贝叶斯4_采样近似推理_笔记|《人工智能原理》采样近似推理]] 中，Gibbs 采样被用于**贝叶斯网络**的近似推理：每步按变量的**马尔科夫覆盖（Markov blanket）**重采样，与本章「固定其余参数、按条件分布逐个采样」本质相同。该课程还把它与直接采样 / 拒绝采样 / 似然加权放在一起作演进对比——一个面向离散网络推断、一个面向连续参数后验，可互为补充。
